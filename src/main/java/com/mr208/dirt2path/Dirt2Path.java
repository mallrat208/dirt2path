...
@Mod(modid = Dirt2Path.MOD_ID, name = Dirt2Path.MOD_NAME, version = "1.7.1", acceptableRemoteVersions = "*", acceptedMinecraftVersions = "[1.12.2]")
public class Dirt2Path
{
	...
	protected boolean isBlockPath(IBlockState iBlockStateIn)
	{
		...
		if(iBlockStateIn.getBlock()==Blocks.GRASS_PATH)
			return true;
		if(flattenBOP&&(iBlockStateIn.getBlock()==BOP_GRASS_PATH))
			return true;
		return false;
	}
	...
	{
		int blockMeta=iBlockStateIn.getBlock().getMetaFromState(iBlockStateIn);
		if(iBlockStateIn.getBlock()==Blocks.DIRT)
			return true;
		if(iBlockStateIn.getBlock()==Blocks.MYCELIUM)
			return true;
		if(flattenBOP&&(iBlockStateIn.getBlock()==BOP_DIRT&&blockMeta<4))
			return true;
		if(flattenBOP&&(iBlockStateIn.getBlock()==BOP_GRASS&&blockMeta>4))
			return true;
		if(flattenBotania&&iBlockStateIn.getBlock()==BOTANIA_GRASS_1_11_2)
			return true;
	...
	{
		int blockMeta=iBlockStateIn.getBlock().getMetaFromState(iBlockStateIn);
		if(iBlockStateIn.getBlock()==Blocks.FARMLAND)
			return true;
		if(iBlockStateIn.getBlock()==BOP_FARMLAND_0)
			return true;
		if(iBlockStateIn.getBlock()==BOP_FARMLAND_1)
			return true;
		return false;
	}
